...
export async function terminalConnects(page: Page, input?: { term?: Locator }) {
  ...
  return page.evaluate((id) => {
    return (window as E2EWindow).__noolcode_e2e?.terminal?.terminals?.[id]?.connects ?? 0
  }, id)
}
...
export async function disconnectTerminal(page: Page, input?: { term?: Locator }) {
  ...
  await page.evaluate((id) => {
    ;(window as E2EWindow).__noolcode_e2e?.terminal?.controls?.[id]?.disconnect?.()
  }, id)
}
...
async function terminalReady(page: Page, term?: Locator) {
  ...
  return page.evaluate((id) => {
    const state = (window as E2EWindow).__noolcode_e2e?.terminal?.terminals?.[id]
...
async function terminalHas(page: Page, input: { term?: Locator; token: string }) {
  ...
  return page.evaluate(
    (input) => {
      const state = (window as E2EWindow).__noolcode_e2e?.terminal?.terminals?.[input.id]
...
export async function seedProjects(page: Page, input: { directory: string; extra?: string[] }) {
  await page.addInitScript(
    (args: { directory: string; serverUrl: string; extra: string[] }) => {
      const key = "noolcode.global.dat:server"
...
export async function createTestProject() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "noolcode-e2e-project-"))
