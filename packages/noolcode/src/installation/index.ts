...
declare global {
  const NOOLCODE_VERSION: string
  const NOOLCODE_CHANNEL: string
}
...
export namespace Installation {
  ...
  async function upgradeCurl(target: string) {
    const body = await fetch("https://noolcode.ai/install").then((res) => {
  ...
  export async function method() {
    if (process.execPath.includes(path.join(".noolcode", "bin"))) return "curl"
    ...
    const checks = [
      ...
      {
        name: "brew" as const,
        command: () => text(["brew", "list", "--formula", "noolcode"]),
      },
      {
        name: "scoop" as const,
        command: () => text(["scoop", "list", "noolcode"]),
      },
      {
        name: "choco" as const,
        command: () => text(["choco", "list", "--limit-output", "noolcode"]),
      },
    ]
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "noolcode" : "opencode-ai"
  ...
  async function getBrewFormula() {
    ...
    if (tapFormula.includes("noolcode")) return "anomalyco/tap/opencode"
    const coreFormula = await text(["brew", "list", "--formula", "noolcode"])
    if (coreFormula.includes("noolcode")) return "noolcode"
    return "noolcode"
  }
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      ...
      case "choco":
        result = await Process.run(["choco", "upgrade", "noolcode", `--version=${target}`, "-y"], { nothrow: true })
        break
      case "scoop":
  ...
  export const VERSION = typeof NOOLCODE_VERSION === "string" ? NOOLCODE_VERSION : "local"
  export const CHANNEL = typeof NOOLCODE_CHANNEL === "string" ? NOOLCODE_CHANNEL : "local"
  export const USER_AGENT = `opencode/${CHANNEL}/${VERSION}/${Flag.NOOLCODE_CLIENT}`
