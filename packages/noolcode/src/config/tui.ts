...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.NOOLCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.NOOLCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.NOOLCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".noolcode") && dir !== Flag.NOOLCODE_CONFIG_DIR) continue
