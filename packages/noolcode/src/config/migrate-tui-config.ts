...
const TUI_SCHEMA_URL = "https://noolcode.ai/tui.json"
...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.NOOLCODE_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("noolcode", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "noolcode")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "noolcode"))
  }
  if (Flag.NOOLCODE_CONFIG) files.push(Flag.NOOLCODE_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "noolcode"))
