...
declare const NOOLCODE_LIBC: string | undefined
...
export namespace FileWatcher {
  ...
  const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
    try {
      const binding = require(
        `@parcel/watcher-${process.platform}-${process.arch}${process.platform === "linux" ? `-${NOOLCODE_LIBC || "glibc"}` : ""}`,
      )
  ...
  const state = Instance.state(
    async () => {
      ...
      if (Flag.NOOLCODE_EXPERIMENTAL_FILEWATCHER) {
  ...
  export function init() {
    if (Flag.NOOLCODE_EXPERIMENTAL_DISABLE_FILEWATCHER) {
      return
    }
    state()
  }
