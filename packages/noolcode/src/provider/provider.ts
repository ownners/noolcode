...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async opencode(input) {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["noolcode"]?.options?.apiKey) return true
        return false
      })()
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://noolcode.ai/",
            "X-Title": "noolcode",
          },
        },
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "noolcode",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [id, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.NOOLCODE_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
  ...
  export async function getSmallModel(providerID: ProviderID) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("noolcode")) {
        priority = ["gpt-5-nano"]
      }
