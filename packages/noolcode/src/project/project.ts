...
import { fn } from "@noolcode/util/fn"
...
export namespace Project {
  ...
  function readCachedId(dir: string) {
    return Filesystem.readText(path.join(dir, "noolcode"))
      .then((x) => x.trim())
      .then(ProjectID.make)
      .catch(() => undefined)
  }
  ...
  export async function fromDirectory(directory: string) {
      ...
      if (dotgit) {
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox,
            vcs: Info.shape.vcs.parse(Flag.NOOLCODE_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          if (id) {
            ...
            await Filesystem.write(path.join(worktree, ".git", "noolcode"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.NOOLCODE_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
