...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@noolcode/sdk/v2"
...
export namespace ACP {
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const opencodeProvider = providers.find((p) => p.id === "noolcode")
    if (opencodeProvider) {
      if (opencodeProvider.models["big-pickle"]) {
        return { providerID: ProviderID.noolcode, modelID: ModelID.make("big-pickle") }
      }
