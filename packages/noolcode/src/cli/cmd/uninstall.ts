...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "opencode-ai"],
      yarn: ["yarn", "global", "remove", "opencode-ai"],
      brew: ["brew", "uninstall", "noolcode"],
      choco: ["choco", "uninstall", "noolcode"],
      scoop: ["scoop", "uninstall", "noolcode"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "noolcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".noolcode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".noolcode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".noolcode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".noolcode"))
    ) {
      continue
    }
