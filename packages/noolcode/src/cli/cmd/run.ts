...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@noolcode/sdk/v2"
...
export const RunCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NOOLCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.NOOLCODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.NOOLCODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.NOOLCODE_SERVER_USERNAME ?? "noolcode"
