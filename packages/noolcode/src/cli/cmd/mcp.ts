...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .noolcode/ subdirectory too)
  const candidates = [path.join(baseDir, "noolcode.json"), path.join(baseDir, "noolcode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".noolcode", "noolcode.json"), path.join(baseDir, ".noolcode", "noolcode.jsonc"))
  }
