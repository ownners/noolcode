...
describe("file/time", () => {
  ...
  describe("assert()", () => {
    ...
    test("skips check when NOOLCODE_DISABLE_FILETIME_CHECK is true", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "file.txt")
      await fs.writeFile(filepath, "content", "utf-8")
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.NOOLCODE_DISABLE_FILETIME_CHECK
          ;(Flag as { NOOLCODE_DISABLE_FILETIME_CHECK: boolean }).NOOLCODE_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { NOOLCODE_DISABLE_FILETIME_CHECK: boolean }).NOOLCODE_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
