...
const managedConfigDir = process.env.NOOLCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "noolcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "noolcode.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://noolcode.ai/config.json",
      snapshot: false,
    })
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "noolcode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["NOOLCODE_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      opencode: {
        options: {
          apiKey: "{env:NOOLCODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["noolcode"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["NOOLCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["NOOLCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "noolcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .noolcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".noolcode")
...
test("loads agents from .noolcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".noolcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .noolcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".noolcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .noolcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".noolcode")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("does not try to install dependencies in read-only NOOLCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.NOOLCODE_CONFIG_DIR
  process.env.NOOLCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.NOOLCODE_CONFIG_DIR
    else process.env.NOOLCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable NOOLCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "noolcode.json"),
        JSON.stringify({ $schema: "https://noolcode.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "noolcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .noolcode config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".noolcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .noolcode config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "noolcode.json"),
        JSON.stringify({
          $schema: "https://noolcode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .noolcode config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "noolcode.json"),
        JSON.stringify({
          $schema: "https://noolcode.ai/config.json",
...
// Note: preload.ts sets NOOLCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .noolcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .noolcode directory config enables it
      const opencodeDir = path.join(dir, ".noolcode")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "noolcode.json"),
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.noolcode/plugin/oh-my-opencode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.noolcode/plugin/oh-my-opencode.js")
  })
...
describe("NOOLCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"]
    process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .noolcode/ directories when flag is set", async () => {
    const originalEnv = process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"]
    process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .noolcode directory with a command
          const opencodeDir = path.join(dir, ".noolcode", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .noolcode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NOOLCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NOOLCODE_CONFIG_DIR"]
      process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NOOLCODE_CONFIG_DIR"]
      } else {
        process.env["NOOLCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NOOLCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NOOLCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NOOLCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NOOLCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from NOOLCODE_CONFIG_DIR, not project
...
describe("NOOLCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in NOOLCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NOOLCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["NOOLCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://noolcode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["NOOLCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["NOOLCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in NOOLCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NOOLCODE_CONFIG_CONTENT"]
