...
import "./styles.css"
import { useTheme } from "@noolcode/ui/theme"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NOOLCODE__ ??= {}
  const pending = window.__NOOLCODE__.deepLinks ?? []
  window.__NOOLCODE__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__NOOLCODE__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__NOOLCODE__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__NOOLCODE__?.wsl) return result
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://noolcode.ai/favicon-96x96-v3.png",
      })
    ...
    getWslEnabled: async () => {
      ...
      return window.__NOOLCODE__!.wsl ?? false
    },
