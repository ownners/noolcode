...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.noolcode.desktop.dev"
}
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"noolcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // opencode.settings.dat → the electron settings store ("noolcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "noolcode.global.dat").
  const storeName = filename === "noolcode.settings.dat" ? "noolcode.settings" : filename
