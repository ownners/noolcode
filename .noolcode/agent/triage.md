...
- `noolcode web` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `noolcode web` view. **Do not** add for terminal, TUI, or general noolcode issues.
...
**Only** add if the issue mentions "zen" or "noolcode zen" or "noolcode black".

If the issue doesn't have "zen" or "noolcode black" in it then don't add zen label

#### core

Use for core server issues in `packages/noolcode/`, excluding `packages/noolcode/src/cli/cmd/tui/`.
...
TUI (`packages/noolcode/src/cli/cmd/tui/...`):
...
Core (`packages/noolcode/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for noolcode server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
